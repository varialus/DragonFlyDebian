...
(cd ${tmp} && diff -x .svn -x -Nurd null sysdeps/ ) > debian/patches/kfreebsd-sysdeps.diff
(cd ${tmp} && diff -x .svn -x -Nurd null linuxthreads/ ) >> debian/patches/kfreebsd-sysdeps.diff
rm -rf ${tmp}
